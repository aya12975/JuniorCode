...
<title>Delete Class | JuniorCode Admin</title>
...
.panel-card { background: white; border: 1px solid #fecaca; border-radius: 22px; padding: 28px; box-shadow: var(--shadow); max-width: 600px; }
...
.info-label { font-weight: 800; color: var(--muted); min-width: 90px; }
...
<div class="app-shell">
  ...
  <main class="main-content">
    ...
    <div class="topbar">
      <div>
        <h1><i class="fas fa-trash me-2"></i>Delete Class</h1>
    ...
    <div class="panel-card">
      ...
      <div class="info-row">
        <span class="info-label"><i class="fas fa-chalkboard-teacher me-1"></i> Teacher</span>
        <span class="info-val"><?= htmlspecialchars($class["teacher_name"]) ?></span>
      </div>
      <div class="info-row">
        <span class="info-label"><i class="fas fa-user me-1"></i> Student</span>
        <span class="info-val"><?= htmlspecialchars($class["student_name"]) ?></span>
      </div>
      <div class="info-row">
        <span class="info-label"><i class="fas fa-calendar me-1"></i> Date</span>
        <span class="info-val"><?= htmlspecialchars($class["class_date"]) ?></span>
      </div>
      <div class="info-row">
        <span class="info-label"><i class="fas fa-clock me-1"></i> Time</span>
        <span class="info-val"><?= htmlspecialchars($class["class_time"]) ?></span>
      </div>
      <div class="info-row">
        <span class="info-label"><i class="fas fa-tag me-1"></i> Type</span>
        <span class="type-badge"><?= htmlspecialchars($class["type"]) ?></span>
      </div>
      <?php if (!empty($class["details"])): ?>
      <div class="info-row">
        <span class="info-label"><i class="fas fa-note-sticky me-1"></i> Details</span>
        <span class="info-val"><?= htmlspecialchars($class["details"]) ?></span>
      </div>
      <?php endif; ?>

      <div class="warn-box">
        <i class="fas fa-exclamation-circle" style="font-size:1.2rem;flex-shrink:0"></i>
        You are about to permanently delete this class record. This cannot be undone.
      </div>

      <form method="POST" style="display:flex;gap:10px;">
        <button type="submit" class="btn-delete"><i class="fas fa-trash me-1"></i> Yes, Delete</button>
        <a href="manage_classes.php" class="btn-back"><i class="fas fa-arrow-left me-1"></i> Cancel</a>
      </form>
    </div>
  </main>
</div>
