...
$stmt = $conn->prepare("SELECT id, username, role, email, zoom_personal_link, profile_picture FROM users WHERE id = ?");
...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    if (($_POST["action"] ?? "") === "delete_picture") {
        ...
        header("Location: teacher_profile.php?deleted=1");
        exit();
        ...
        if ($file["error"] !== UPLOAD_ERR_OK) {
            ...
            if (move_uploaded_file($file["tmp_name"], $dest)) {
                ...
                header("Location: teacher_profile.php?uploaded=1");
                exit();
            } else {
...
.teacher-box { display: flex; align-items: center; gap: 12px; padding: 12px; border-radius: 14px; background: rgba(255,255,255,0.06); border: 1px solid rgba(255,255,255,0.08); margin-bottom: 18px; }
.teacher-avatar { width: 44px; height: 44px; border-radius: 50%; background: linear-gradient(135deg, var(--primary), var(--secondary)); color: #fff; display: flex; align-items: center; justify-content: center; font-weight: 900; font-size: 18px; flex-shrink: 0; overflow: hidden; }
...
.info-val   { font-weight: 700; color: var(--dark); margin-top: 1px; word-break: break-all; }
...
<div class="sidebar">
  <div class="brand-wrap">
    ...
    <div>
      ...
      <p class="brand-subtitle">TEACHER PANEL</p>
    </div>
  </div>
...
<div class="main">
  ...
  <div class="profile-grid">
    ...
    <div class="panel">
      ...
      <div class="avatar-wrap">
        <div class="big-avatar" id="bigAvatar">
          ...
          <?php else: ?>
            <span id="avatarLetter"><?= strtoupper(substr($teacherName, 0, 1)) ?></span>
          <?php endif; ?>
        </div>
        <div class="avatar-name"><?= htmlspecialchars($user["username"]) ?></div>
        <div class="avatar-role">Teacher</div>
    ...
    <div class="panel">
      ...
      <?php if (!empty($user["zoom_personal_link"])): ?>
      <div class="info-row">
        <div class="info-icon"><i class="fas fa-video"></i></div>
        <div style="min-width:0;flex:1">
          <div class="info-label">Personal Zoom Link</div>
          <div class="info-val">
            <a href="<?= htmlspecialchars($user["zoom_personal_link"]) ?>" target="_blank"
               style="color:#2563eb;text-decoration:none;font-weight:700;">
              <i class="fas fa-external-link-alt me-1" style="font-size:0.75rem"></i>Open Zoom
            </a>
          </div>
        </div>
      </div>
