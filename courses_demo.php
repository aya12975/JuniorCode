...
if ($_SERVER["REQUEST_METHOD"] === "POST" && ($_POST["action"] ?? "") === "add_course") {
    ...
    $type     = "demo";
    if ($name !== "") {
        $stmt = $conn->prepare("INSERT INTO courses (course_name, section, category, price, course_type, status) VALUES (?, 'demo', ?, ?, ?, 'active')");
        if ($stmt) { $stmt->bind_param("ssds", $name, $category, $price, $type); $stmt->execute(); $stmt->close(); }
    }
    header("Location: courses_demo.php?success=1");
    exit();
}
...
if ($_SERVER["REQUEST_METHOD"] === "POST" && ($_POST["action"] ?? "") === "add_project") {
    ...
    header("Location: courses_demo.php?selected=" . $back . "&proj_added=1");
    exit();
}
...
if ($_SERVER["REQUEST_METHOD"] === "POST" && ($_POST["action"] ?? "") === "edit_project") {
    ...
    header("Location: courses_demo.php?selected=" . $courseId . "&proj_edited=1");
    exit();
}
...
if ($_SERVER["REQUEST_METHOD"] === "POST" && ($_POST["action"] ?? "") === "delete_course") {
    ...
    if ($id > 0) {
        ...
        $del = $conn->prepare("DELETE FROM courses WHERE id = ? AND section = 'demo'");
...
if ($_SERVER["REQUEST_METHOD"] === "POST" && ($_POST["action"] ?? "") === "delete_project") {
    ...
    header("Location: courses_demo.php?selected=" . $courseId);
    exit();
}

// ── Fetch ────────────────────────────────────────────────────────────────────
$allCourses = [];
$res = $conn->query("SELECT * FROM courses WHERE section='demo' ORDER BY category ASC, course_name ASC");
if ($res) $allCourses = $res->fetch_all(MYSQLI_ASSOC);

$existCats = [];
$cr = $conn->query("SELECT DISTINCT category FROM courses WHERE section='demo' AND category != '' ORDER BY category ASC");
...
<head>
  ...
  <title>Demo Courses | JuniorCode Admin</title>
...
<div class="app-shell">
  ...
  <main class="main-content">
    ...
    <div class="topbar">
      <div>
        <h1><i class="fas fa-play-circle me-2"></i>Demo Courses</h1>
        <p>Add free trial sessions and manage their projects.</p>
      </div>
    ...
    <div class="card-box">
      <div class="card-box-title">
        <span class="title-icon"><i class="fas fa-plus"></i></span> Add New Demo Course
      </div>
      <form method="POST">
        ...
        <div class="input-row">
          <div>
            ...
            <input type="text" name="course_name" class="form-control" placeholder="e.g. Intro to Scratch" required>
          </div>
          <div>
            <label class="field-label">Group / Level <span style="color:#94a3b8;font-weight:400;">(optional)</span></label>
            <input type="text" name="category" class="form-control" placeholder="e.g. Little, Junior" list="cat-list">
      ...
      <?php else: ?>
        <form method="GET" class="selector-wrap" style="margin-bottom:24px;">
          <select name="selected" class="form-select course-select">
            <option value="">— Select a Demo Course —</option>
            <?php
              $grouped = [];
              foreach ($allCourses as $c) {
                  $key = $c["category"] ?: "No Group";
                  $grouped[$key][] = $c;
              }
        ...
        <div class="proj-panel">
          ...
          <?php else: ?>
            ...
            <div class="proj-item">
              ...
              <?php else: ?>
                <div class="proj-thumb-fallback"><i class="fas fa-play-circle"></i></div>
              <?php endif; ?>
            ...
            <form method="POST" enctype="multipart/form-data">
              ...
              <div class="form-grid-2" style="margin-bottom:12px;">
                <div>
                  ...
                  <input type="text" name="title" class="form-control" placeholder="e.g. My First Scratch Project" required>
                </div>
                <div>
        ...
        <div style="margin-top:28px;">
          <div style="font-size:0.82rem;font-weight:900;text-transform:uppercase;letter-spacing:1px;color:#3e5077;margin-bottom:12px;">All Demo Courses (<?= count($allCourses) ?>)</div>
            ...
            <?php $n=1; foreach ($grouped as $grpLabel => $grpItems): ?>
              ...
              <div class="course-item">
                ...
                <div style="display:flex;gap:6px;align-items:center;flex-shrink:0;flex-wrap:wrap;">
                  ...
                  <a href="courses_demo.php?selected=<?= $c['id'] ?>" class="btn-icon btn-edit" style="width:auto;padding:0 12px;font-size:0.78rem;font-weight:800;gap:5px;text-decoration:none;"><i class="fas fa-folder-open"></i> Projects</a>
...
<div id="edit-proj-modal" style="display:none;position:fixed;inset:0;background:rgba(15,23,42,0.45);z-index:10000;align-items:center;justify-content:center;">
  <div style="background:white;border-radius:22px;padding:36px;max-width:520px;width:94%;box-shadow:0 24px 60px rgba(0,0,0,0.18);">
    <div style="font-size:1.1rem;font-weight:900;margin-bottom:20px;"><i class="fas fa-pen me-2" style="color:var(--primary);"></i>Edit Project</div>
    <form method="POST" enctype="multipart/form-data" id="edit-proj-form">
      <input type="hidden" name="action"     value="edit_project">
      <input type="hidden" name="project_id" id="ep-id">
      <input type="hidden" name="course_id"  id="ep-course-id">
      <input type="hidden" name="existing_pdf" id="ep-existing-pdf">
      <div class="form-grid-2" style="margin-bottom:12px;">
        <div>
          <label class="field-label">Project Title <span style="color:#dc2626;">*</span></label>
          <input type="text" name="title" id="ep-title" class="form-control" required>
        </div>
        <div>
          <label class="field-label">Image URL</label>
          <input type="text" name="image" id="ep-image" class="form-control" placeholder="https://...">
        </div>
      </div>
      <div class="form-grid-2" style="margin-bottom:12px;">
        <div>
          <label class="field-label">Project Link</label>
          <input type="text" name="url" id="ep-url" class="form-control" placeholder="https://...">
        </div>
        <div>
          <label class="field-label">PDF Link</label>
          <input type="text" name="pdf_link" id="ep-pdf-link" class="form-control" placeholder="https://drive.google.com/...">
        </div>
      </div>
      <div style="margin-bottom:18px;">
        <label class="field-label">Or Upload New PDF</label>
        <input type="file" name="pdf_file" accept=".pdf" class="form-control">
      </div>
      <div style="display:flex;gap:10px;justify-content:flex-end;">
        <button type="submit" class="btn-primary-custom"><i class="fas fa-save me-1"></i> Save Changes</button>
        <button type="button" onclick="document.getElementById('edit-proj-modal').style.display='none'" style="background:#64748b;color:white;border:none;border-radius:12px;padding:12px 20px;font-weight:800;cursor:pointer;">Cancel</button>
      </div>
    </form>
  </div>
</div>
...
function openEditProject(pid, cid, title, url, image, pdfUrl) {
  ...
  document.getElementById('ep-course-id').value = cid;
  ...
  document.getElementById('ep-pdf-link').value = pdfUrl.startsWith('http') ? pdfUrl : '';
  document.getElementById('ep-existing-pdf').value = pdfUrl;
