...
if ($_SERVER["REQUEST_METHOD"] === "POST" && ($_POST["action"] ?? "") === "add_course") {
    ...
    if ($name !== "") {
        $stmt = $conn->prepare("INSERT INTO courses (course_name, section, category, price, course_type, status) VALUES (?, 'kids', ?, ?, ?, 'active')");
        if ($stmt) { $stmt->bind_param("ssds", $name, $category, $price, $type); $stmt->execute(); $stmt->close(); }
    }
    header("Location: courses_kids.php?success=1");
    exit();
}
...
if ($_SERVER["REQUEST_METHOD"] === "POST" && ($_POST["action"] ?? "") === "add_project") {
    ...
    header("Location: courses_kids.php?selected=" . $back . "&proj_added=1");
    exit();
}
...
if ($_SERVER["REQUEST_METHOD"] === "POST" && ($_POST["action"] ?? "") === "edit_project") {
    ...
    header("Location: courses_kids.php?selected=" . $courseId . "&proj_edited=1");
    exit();
}
...
if ($_SERVER["REQUEST_METHOD"] === "POST" && ($_POST["action"] ?? "") === "delete_course") {
    ...
    if ($id > 0) {
        ...
        $del = $conn->prepare("DELETE FROM courses WHERE id = ? AND section = 'kids'");
...
if ($_SERVER["REQUEST_METHOD"] === "POST" && ($_POST["action"] ?? "") === "delete_project") {
    ...
    header("Location: courses_kids.php?selected=" . $courseId);
    exit();
}

// ── Fetch ────────────────────────────────────────────────────────────────────
$allCourses = [];
$res = $conn->query("SELECT * FROM courses WHERE section='kids' ORDER BY category ASC, course_name ASC");
if ($res) $allCourses = $res->fetch_all(MYSQLI_ASSOC);

$existCats = [];
$cr = $conn->query("SELECT DISTINCT category FROM courses WHERE section='kids' AND category != '' ORDER BY category ASC");
...
<head>
  ...
  <title>Kids Courses | JuniorCode Admin</title>
...
<div class="app-shell">
  ...
  <main class="main-content">
    ...
    <div class="topbar">
      <div>
        <h1><i class="fas fa-child me-2"></i>Kids Courses</h1>
    ...
    <div class="card-box">
      <div class="card-box-title">
        <span class="title-icon"><i class="fas fa-plus"></i></span> Add New Kids Course
      </div>
      <form method="POST">
        ...
        <div class="input-row">
          <div>
            ...
            <input type="text" name="course_name" class="form-control" placeholder="e.g. Scratch Game Design" required>
          </div>
          <div>
            <label class="field-label">Module / Category</label>
            <input type="text" name="category" class="form-control" placeholder="e.g. Game Development" list="cat-list">
      ...
      <?php else: ?>
        <form method="GET" class="selector-wrap" style="margin-bottom:24px;">
          <select name="selected" class="form-select course-select">
            <option value="">— Select a Kids Course —</option>
            <?php
              $grouped = [];
              foreach ($allCourses as $c) {
        ...
        <!-- ── Projects Panel (shown when a course is selected) ── -->
        ...
        <div class="proj-panel">
          ...
          <?php else: ?>
            ...
            <div class="proj-item">
              ...
              <?php else: ?>
                <div class="proj-thumb-fallback"><i class="fas fa-gamepad"></i></div>
              <?php endif; ?>
            ...
            <form method="POST" enctype="multipart/form-data">
              ...
              <div class="form-grid-2" style="margin-bottom:12px;">
                <div>
                  ...
                  <input type="text" name="title" class="form-control" placeholder="e.g. Dino Run Game" required>
                </div>
                <div>
        ...
        <div style="margin-top:28px;">
          <div style="font-size:0.82rem;font-weight:900;text-transform:uppercase;letter-spacing:1px;color:#3e5077;margin-bottom:12px;">All Kids Courses (<?= count($allCourses) ?>)</div>
            ...
            <?php $n=1; foreach ($grouped as $grpLabel => $grpItems): ?>
              ...
              <div class="course-item">
                ...
                <div style="display:flex;gap:6px;align-items:center;flex-shrink:0;flex-wrap:wrap;">
                  ...
                  <a href="courses_kids.php?selected=<?= $c['id'] ?>" class="btn-icon btn-edit" style="width:auto;padding:0 12px;font-size:0.78rem;font-weight:800;gap:5px;text-decoration:none;"><i class="fas fa-folder-open"></i> Projects</a>
  ...
  <div style="background:white;border-radius:22px;padding:32px;max-width:520px;width:94%;box-shadow:0 24px 60px rgba(0,0,0,0.18);">
    ...
    <form method="POST" enctype="multipart/form-data" id="edit-proj-form">
      ...
      <div class="form-grid-2" style="margin-bottom:12px;">
        <div>
          <label class="field-label">Project Link <span style="color:#94a3b8;font-weight:400;">(optional)</span></label>
          <input type="text" name="url" id="edit-proj-url" class="form-control" placeholder="https://scratch.mit.edu/...">
        </div>
        <div>
          <label class="field-label">PDF Link <span style="color:#94a3b8;font-weight:400;">(optional)</span></label>
